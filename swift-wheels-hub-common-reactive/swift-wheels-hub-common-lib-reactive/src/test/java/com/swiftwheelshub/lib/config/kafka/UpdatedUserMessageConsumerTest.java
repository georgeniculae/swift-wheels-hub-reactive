...
class UpdatedUserMessageConsumerTest {

    @InjectMocks
    private UpdatedUserMessageConsumer updatedUserMessageConsumer;
    ...
    void userConsumerTest_success() {
        ...
        when(userService.processUpdatedUser(any(UserDto.class))).thenReturn(Mono.just(userDto));

        StepVerifier.create(updatedUserMessageConsumer.updatedUserConsumer().apply(flux))
                .expectNext()
                .verifyComplete();
    }
    ...
    void userConsumerTest_exceptionOnProcessMessage() {
        ...
        when(userService.processUpdatedUser(any(UserDto.class))).thenReturn(Mono.error(new Throwable()));
    ...
    void userConsumerTest_successAcknowledgedMessageWithHeaders() {
        ReflectionTestUtils.setField(updatedUserMessageConsumer, "isMessageAckEnabled", true);
